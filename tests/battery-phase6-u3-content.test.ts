...
test('Fase 6 U3 publica overview, cuatro lecciones y evaluación',()=>{
  ...
  assert.match(docs[5],/title: "Evaluación — Navegación y decisión"/);
});

test('U3 profundiza la jerarquía de lectura sin repetir la lectura mínima de Fase 5',()=>{
...
test('La evaluación prioriza continuidad, recuperación y evita puertas falsas',()=>{
  const cp=docs[5];
  ...
  assert.match(docs[0],/no se convierte en un bloque fijo de mantenimiento/i);
  assert.match(cp,/sustituya una tarea secundaria/i);
});
