...
function summary(nextAction: PracticeExecution['nextAction'], contentId = 'bat-f1-u1-l1'): CourseProgressSummary {
  return {
    courseId: 'bateria',
    executionCount: 1,
    completedContentIds: nextAction === 'repeat' || nextAction === 'stop' ? [] : [contentId],
    lastExecution: {
      id: 'execution-1',
      userId: 1,
      courseId: 'bateria',
...
test('continue destination starts at the first real course content without progress', () => {
  assert.equal(
    resolveContinueDestination(undefined, items, '/bateria/'),
    '/bateria/unidad-1/diagnostico/',
  );
});

test('continue and continue_review advance through the curricular order', () => {
  assert.equal(
    resolveContinueDestination(summary('continue'), items, '/bateria/'),
    '/bateria/unidad-1/leccion-2/',
  );
  assert.equal(
    resolveContinueDestination(summary('continue_review'), items, '/bateria/'),
    '/bateria/unidad-1/leccion-2/',
  );
});
...
test('repeat returns to the same content and stop falls back to the course entry', () => {
  assert.equal(
    resolveContinueDestination(summary('repeat'), items, '/bateria/'),
    '/bateria/unidad-1/leccion-1/',
  );
  assert.equal(
    resolveContinueDestination(summary('stop'), items, '/bateria/'),
    '/bateria/',
  );
});
...
test('unknown historical content degrades to the first unfinished known content', () => {
  ...
  assert.equal(
    resolveContinueDestination(value, items, '/bateria/'),
    '/bateria/unidad-1/leccion-1/',
  );
});
