...
test('U8 L3 renders one traceable transformation', async ({ page }, testInfo) => {
  ...
  await expectOneScoreReady(page);
  const article = page.locator('article.course-article');
  await expect(article.getByText(/una sola transformación desarrollo motívico consciente/i)).toBeVisible();
...
test('U8 Checkpoint 3B leaves three bars open and does not claim final Hito 4', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('heading', { level: 1, name: 'Evaluación — Autoría en desarrollo' })).toBeVisible();
  await expectOneScoreReady(page);
  const article = page.locator('article.course-article');
  ...
  await expect(article.getByText(/AVANZADO no es requisito para Unidad 9/)).toBeVisible();
});
