...
test('Phase 2 U7 overview keeps metric recognition explicitly initial and bounded', async () => {
  const markdown = await readFile(overviewPath, 'utf8');

  assert.match(markdown, /El reconocimiento métrico entra sólo de forma inicial/i);
  ...
  assert.match(markdown, /declarar el reconocimiento de métrica, frase y forma globalmente `FUNCIONAL` por completar esta unidad/i);
});
...
test('Phase 2 U7 overview fixes the four-lesson route and protects later-unit boundaries', async () => {
  ...
  assert.match(markdown, /sextillos y ornamentación escrita como vocabulario central — Unidad 8/);
