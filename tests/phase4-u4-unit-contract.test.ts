...
test('overview keeps hi-hat parallel to kick and defines the approved minimum', async () => {
  const overview = plain(await page('overview'));
  assert.match(overview, /bombo y hi-hat de pie como ramas paralelas/i);
...
test('L2 treats heel choices as pedagogy and partial opening as expansion', async () => {
  ...
  assert.match(l2, /no una lección de openings estilísticos ni un requisito de hi-hat de pie MÍNIMO/i);
...
test('L4 layers known hands with left foot without premature four-limb certification', async () => {
  ...
  assert.match(l4, /no hace falta haber completado bombo/i);
  assert.match(l4, /no certifica coordinación básica de cuatro extremidades ni independencia avanzada/i);
});

test('checkpoint certifies only hi-hat-foot minimum and leaves later skills open', async () => {
  const cp = plain(await page('checkpoint'));
  assert.match(cp, /hi-hat de pie alcanza el MÍNIMO PARA AVANZAR/i);
  ...
  for (const excluded of ['técnica de bombo', 'coordinación básica de cuatro extremidades', 'groove funcional', 'independencia', 'foot splash funcional', 'ostinatos variados o rápidos']) {
    assert.match(cp, new RegExp(excluded, 'i'));
  }
  assert.match(cp, /No es requisito para iniciar Unidad 5/i);
  assert.match(cp, /La perfección no es requisito para continuar/i);
  assert.match(cp, /no se mantienen como dos deberes diarios independientes/i);
  assert.match(cp, /no crea un bloque diario adicional de pedal/i);
});
...
test('U3 remains a kick-only checkpoint when U4 introduces left foot', async () => {
  const u3 = plain(await readFile(path.join(pagesRoot, 'f4-u3-checkpoint-bombo-primera-voz-pie.md'), 'utf8'));
  assert.match(u3, /bombo está suficientemente disponible al nivel mínimo/i);
  assert.match(u3, /hi-hat de pie/);
  assert.match(u3, /coordinación básica de cuatro extremidades/);
});
