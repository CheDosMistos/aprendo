...
test('overview rejects generic Latin and explains why no local idiomatic score is fabricated', async()=>{ const t=plain(await page('overview')); assert.match(t,/BRAZILIAN ≠ AFRO-CUBAN ≠ “LATIN GENÉRICO”/i); assert.match(t,/categoría paraguas/i); assert.match(t,/Por qué no hay una partitura local “latina”/i); assert.match(t,/NO ES UN PATRÓN TRADICIONAL NI UNA TRANSCRIPCIÓN/i); assert.match(t,/no crean dos rutinas estilísticas permanentes/i); assert.match(t,/sustituya una tarea secundaria/i); assert.match(t,/No existe BPM de aprobado/i); });
...
test('L2 labels its samba layer lab as original but not authentic samba', async()=>{ const t=plain(await page('l2')); assert.match(t,/EJERCICIO ORIGINAL CREADO PARA ESTE CURSO/i); assert.match(t,/NO ES UN PATRÓN DE SAMBA AUTÉNTICO/i); assert.match(t,/LABORATORIO DE CAPAS ≠ GROOVE DE SAMBA/i); assert.match(t,/surdo/i); assert.match(t,/caixa/i); assert.match(t,/Independencia contextual/i); assert.doesNotMatch(t,/independencia avanzada.*contextual/i); });

test('L3 learns clave from external verified source and keeps neutral layer separate', async()=>{ const t=plain(await page('l3')); assert.match(t,/2-3 son clave/i); assert.match(t,/CLAVE ≠ ADORNO RÍTMICO/i); assert.match(t,/Berklee PULSE/i); assert.match(t,/Aprendo no reconstruye aquí la partitura/i); assert.match(t,/no se presenta como tumbao, cáscara ni patrón de mambo/i); assert.match(t,/Independencia contextual/i); assert.doesNotMatch(t,/independencia avanzada.*contextual/i); });

test('L4 compares without hybridizing and closes U9 without checkpoint or maintenance debt', async()=>{ const t=plain(await page('l4')); assert.match(t,/TRANSFERIR ≠ MEZCLAR/i); assert.match(t,/Brazilian termina — Afro-Cuban empieza/i); assert.match(t,/no fusiona los dos idiomas/i); assert.match(t,/tampoco mantengas ambos laboratorios por obligación/i); assert.match(t,/No hay (?:una evaluación|un checkpoint) global adicional/i); assert.match(t,/no existe BPM de aprobado/i); });
