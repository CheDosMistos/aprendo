...
test('recording an execution appends history and normalizes optional text', () => {
  ...
  try {
    const execution = service.recordExecution({
      ...
      problem: '  El segundo golpe del double pierde claridad.  ', note: '  ',
    });
...
test('partial is review state, not completed, and latest execution wins', () => {
  ...
  try {
    ...
    assert.deepEqual(summary.needsReviewContentIds, ['bat-f1-u1-l1']);
    ...
    assert.deepEqual(summary.needsReviewContentIds, []);
...
test('skill evidence keeps history and current state separate', () => {
  ...
  let id = 1;
  let minute = 0;
  const skills = new SkillEvidenceService(database, {
    createId: () => `evidence-${id++}`,
    now: () => new Date(`2026-08-21T08:${String(minute++).padStart(2, '0')}:00.000Z`),
  });
  try {
    let state = skills.record({
      courseId: 'bateria', skillType: 'rudiment', skillId: 'Flam', observation: 'unstable',
      workState: 'stabilizing', retentionState: 'not_checked', limitingVariable: 'movement', corrective: 'Preparar alturas y retestar.',
    });
    assert.equal(state.workState, 'stabilizing');
    assert.equal(state.masteryState, null);

    state = skills.record({
      courseId: 'bateria', skillType: 'rudiment', skillId: 'Flam', observation: 'available',
      masteryState: 'known', retentionState: 'checked',
    });
    assert.equal(state.masteryState, 'known');
    assert.equal(state.retentionState, 'checked');
    assert.equal(skills.listStates('bateria', 'rudiment').length, 1);
    const count = database.prepare('SELECT COUNT(*) AS count FROM skill_evidence').get() as { count: number };
