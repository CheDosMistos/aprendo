...
test('overview keeps style as language rather than pattern', async()=>{ const t=plain(await page('overview')); assert.match(t,/tocar menos sin perder referencia ni intención/i); assert.match(t,/ESPACIO ≠ AUSENCIA DE PULSO/i); assert.match(t,/no reduciremos reggae/i); assert.match(t,/Pieza B conserva su identidad shuffle/i); assert.match(t,/no convierte offbeat, one-drop y steppers en tres deberes de mantenimiento/i); assert.match(t,/sustituye una tarea secundaria/i); assert.match(t,/No existe BPM de aprobado/i); });
...
test('L2 uses an original one-drop pedagogical model with explicit boundary', async()=>{ const t=plain(await page('l2')); assert.match(t,/f5-u7-one-drop-steppers\.musicxml/); assert.match(t,/MODELO PEDAGÓGICO ≠ PATRÓN OFICIAL/i); assert.match(t,/tiempo 3/i); assert.match(t,/Independencia contextual/i); assert.doesNotMatch(t,/independencia avanzada contextual/i); assert.match(t,/no pretende copiar/i); });
...
test('L4 continues Piece B without style erasure or maintenance debt', async()=>{ const t=plain(await page('l4')); assert.match(t,/NO CONVIERTAS PIEZA B EN REGGAE/i); assert.match(t,/Pieza B.*shuffle/is); assert.match(t,/no añadas automáticamente un bloque separado/i); assert.match(t,/no añade una evaluación global/i); });
