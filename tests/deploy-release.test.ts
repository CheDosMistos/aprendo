...
function createRelease(runtime: string, name: string, healthy: boolean, mutateDb = false): string {
...
function createWrappers(root: string, runtime: string, dbPath: string): { systemctl: string; health: string } {
...
function baseEnv(runtime: string, dbPath: string, wrappers: { systemctl: string; health: string }): NodeJS.ProcessEnv {
  return {
    ...process.env,
...
test('release activation converts the legacy mutable runtime and switches current atomically', () => {
  ...
  const wrappers = createWrappers(root, runtime, dbPath);

  execFileSync('bash', [activateScript, 'release-ok'], {
    env: baseEnv(runtime, dbPath, wrappers),
    stdio: 'pipe',
  });
...
test('failed release health rolls the current release and SQLite back together', () => {
