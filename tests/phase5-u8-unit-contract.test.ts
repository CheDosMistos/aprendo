...
test('overview protects jazz boundaries and selective Riley use', async()=>{ const t=plain(await page('overview')); assert.match(t,/TIME PRIMERO → RESPUESTA DESPUÉS/i); assert.match(t,/REJILLA TERNARIA ≠ SWING REAL COMPLETO/i); assert.match(t,/COMPING ≠ TOCAR MÁS/i); assert.match(t,/John Riley: recurso selectivo, no currículo/i); assert.doesNotMatch(t,/H7 avanzada contextual/i); assert.match(t,/No existe BPM de aprobado/i); });
...
test('L2 keeps foot hi-hat subordinate to ride time', async()=>{ const t=plain(await page('l2')); assert.match(t,/2.*4/is); assert.match(t,/EL PIE APOYA EL TIME; NO LO SUSTITUYE/i); assert.match(t,/H7 contextual/i); assert.doesNotMatch(t,/H7 avanzada contextual/i); });

test('L3 defines comping as limited musical response, not density', async()=>{ const t=plain(await page('l3')); assert.match(t,/COMPING ≠ TOCAR MÁS/i); assert.match(t,/acompañamiento o conversación rítmica/i); assert.match(t,/H7 contextual/i); assert.doesNotMatch(t,/H7 avanzada contextual/i); assert.match(t,/no requisitos/i); });

test('L4 navigates an original form without creating a checkpoint', async()=>{ const t=plain(await page('l4')); assert.match(t,/TIEMPO → FORMA → ENTRADA → FIGURE → DETALLE/i); assert.match(t,/EJERCICIO ORIGINAL CREADO PARA ESTE CURSO/i); assert.match(t,/A — A — B — A/i); assert.match(t,/FORMA CONSERVADA > RESPUESTA PERFECTA/i); assert.match(t,/No hay (?:una evaluación|un checkpoint) global adicional/i); assert.doesNotMatch(t,/EST-JAZ-001/i); });
