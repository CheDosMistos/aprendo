...
La secuencia de trabajo es obligatoria:

`DECODIFICAR LÍNEA → TOCAR LÍNEA BASE → ELEGIR TEXTURA → APLICAR → COMPARAR`.

Si anticipar un sticking te hace añadir un golpe, rellenar un silencio, reatacar una ligadura o desplazar una posición, la textura ha dejado de servir a la lectura y debe retirarse.
...
1. ataques reales;
2. silencios;
3. continuaciones por ligadura;
4. pulso y subdivisión;
5. continuidad después de un error pequeño.

No marques todavía una solución de manos. Una línea que sólo puedes tocar después de recordar el sticking no ofrece buena evidencia de lectura.
...
### Paso A — comparar antes de tocar

Pon lado a lado la línea base y la versión con textura. Para cada compás, verifica:

- mismo número de posiciones temporales;
- mismos ataques y silencios;
- mismas ligaduras;
- misma duración total;
- único cambio: asignación de mano en los ataques.

Si no puedes demostrar eso visualmente, todavía no apliques la textura.

### Paso B — aplicar por fragmentos con sentido temporal

Elige un compás ya decodificado:

1. tócalo con sticking alternado;
2. tócalo con la distribución `R/L` escrita;
3. conserva exactamente las mismas distancias entre ataques;
4. cuenta o canta la rejilla si una pareja de la misma mano tiende a comprimirse;
5. compara inmediatamente ambas versiones.

Una pareja `RR` o `LL` **no autoriza** a inventar un golpe intermedio. Dos letras de la misma mano corresponden a dos ataques escritos, aunque la separación entre ellos no sea la de un ejercicio rudimental tradicional.

### Paso C — si la textura domina, retírala

Retira la textura y vuelve a la línea base si aparece cualquiera de estas señales:

- rellenas silencios;
- reatacas una ligadura;
- el pulso se desplaza al anticipar una mano;
...
Escoge un compás ya comprendido y construye dos versiones:

- **Versión A:** sticking alternado sencillo;
- **Versión B:** la textura de manos publicada o una variante original que conserve todos los ataques.
...
- mano que lidera;
- presencia de pares `RR/LL`;
- sensación mecánica;
- dificultad relativa de cada mano.

### Qué debe permanecer fijo

- posiciones de ataque;
- silencios;
- duraciones y ligaduras;
- pulso/subdivisión;
- duración del compás;
- continuidad general.

Como transferencia, cambia **una sola pareja de manos** en otro compás. No cambies a la vez acentos, tempo, densidad y sticking. Si la nueva versión altera el resultado rítmico, vuelve al original y localiza exactamente qué ataque fue contaminado por la decisión de manos.
...
- sticking base;
- textura aplicada;
- si ataques/silencios/ligaduras permanecieron idénticos;
- precisión temporal;
- continuidad y recuperación;
- relajación y control;
- igualdad funcional entre manos;
- variable limitante: lectura, tiempo, duración, sticking, double, mano débil, tensión, memoria o carga;
- siguiente acción concreta.

Interpreta con la cadena aprobada:
...
Como ampliación, prepara dos soluciones de sticking para el mismo compás y justifica cuál facilita mejor la continuidad o la bilateralidad **sin modificar el resultado rítmico**. Si la justificación se reduce a «es más rápida», falta información: considera también control, relajación, sonido y estabilidad temporal.

## Si algo se rompe

- **añades golpes para completar una serie de manos:** vuelve al mapa de ataques; una letra sólo puede corresponder a un ataque existente;
- **reatacas una ligadura:** elimina temporalmente las letras y marca sólo los inicios reales;
- **un `RR/LL` se comprime:** reduce tempo y conserva verbalmente la subdivisión entre ambos ataques;
- **el sticking se memoriza antes que la línea:** cambia de compás y vuelve a decodificar antes de asignar manos;
