---
contentId: bat-f3-u11-l5
courseId: bateria
phase: 3
unit: 11
unitSlug: fase-3-unidad-11
slug: take-five-repertorio-documentado
kind: lesson
order: 5
title: "Caso documentado: The Dave Brubeck Quartet — Take Five"
summary: "Aplica mapa, métrica y análisis de capas a una grabación real separando hechos documentados de observaciones propias."
duration: 25–30 min
competencies: [I3, E5, F3, F6, F7, J3, K2, K6, K8]
rudiments: []
published: true
---

## Objetivo

Aprender a trabajar con repertorio real **sin convertir el análisis propio en hecho histórico ni copiar una partitura protegida**.

## EJEMPLO DOCUMENTADO

Para la versión oficial enlazada se ha verificado:

- obra: **“Take Five”**;
- compositor: **Paul Desmond**;
- intérprete: **The Dave Brubeck Quartet**;
- batería: **Joe Morello**;
- bajo: **Eugene Wright**;
- piano: **Dave Brubeck**;
- saxo alto: **Paul Desmond**;
- métrica: **5/4**.

La web oficial de Dave Brubeck documenta que el trabajo de Joe Morello con un beat en 5/4 formó parte de la génesis del tema. La Library of Congress también documenta el 5/4 y el papel de Morello.

### Fuentes verificadas

- [Escucha oficial — Columbia/Legacy en el canal oficial de Dave Brubeck](https://www.youtube.com/watch?v=QsHc2IGmk60)
- [Dave Brubeck — historia oficial de Take Five y Time Out](https://www.davebrubeckjazz.com/take-five-%26-time-out)
- [Library of Congress — Song Stories: Time Out for Take Five](https://blogs.loc.gov/nls-music-notes/2020/05/song-stories-time-out-for-take-five/)

## EJEMPLO ANALÍTICO — tu trabajo

Escucha **esa versión exacta** y construye una ficha propia. No necesitas transcribir el groove nota por nota.

Registra:

1. un mapa de grandes bloques/secciones según lo que oyes;
2. dónde percibes retorno o contraste;
3. cómo te ayuda el 5/4 ya trabajado en U9;
4. una observación sobre continuidad/densidad de batería;
5. una observación sobre la capa de bajo;
6. una relación entre ambas capas que puedas justificar auditivamente;
7. una señal que usarías para anticipar un cambio.

Si no puedes verificar una afirmación histórica, escríbela como **OBSERVACIÓN/INFERENCIA**, no como intención de Morello, Wright, Desmond o Brubeck.

## Copyright

Aprendo enlaza la grabación oficial. **No reproduce aquí la partitura, el audio master ni una transcripción extensa de batería, bajo, armonía o melodía.**

## MÍNIMO PARA AVANZAR

Produces una ficha que separa claramente HECHO DOCUMENTADO de EJEMPLO ANALÍTICO y contiene un mapa útil para volver a escuchar la obra con intención.
