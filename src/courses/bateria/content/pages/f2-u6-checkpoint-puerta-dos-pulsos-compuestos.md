---
contentId: bat-f2-u6-check
courseId: bateria
phase: 2
unit: 6
unitSlug: fase-2-unidad-6
slug: puerta-de-dos-pulsos-compuestos
kind: checkpoint
order: 5
...
## 1. Muestra principal — sentir y leer 6/8 como 2×3 — 5–6 min
...
1. localiza en cada compás **dos pulsos principales**;
2. marca internamente `1-la-li | 2-la-li`, incluyendo las posiciones silenciosas;
3. observa primero la organización `3+3` y después los ataques concretos;
4. realiza **una sola primera tentativa propia** sin playback;
5. si aparece un error local, intenta recuperar la referencia en el siguiente pulso principal seguro en vez de reiniciar por defecto.
...
- el tempo puede reducirse si la condición mezcla demasiadas variables;
...
- **CARGA / RELAJACIÓN**;
- **VARIABLE LIMITANTE:** pulso, subdivisión, lectura, comprensión métrica, memoria, ejecución o carga.

### Comprobación auditiva sobre la misma muestra

Sólo después de tu tentativa, habilita audio y aparta la vista de la partitura durante una escucha.

- marca únicamente **dos apoyos grandes por compás**;
...
Responde con tus palabras. No hace falta repetir una definición literal.
...
La cantidad de corcheas coincide; la organización métrica no.

### ¿Por qué 6/8 no es simplemente «un tresillo»?

En el 6/8 trabajado aquí, las tres corcheas dentro de cada pulso compuesto son **subdivisiones ordinarias del compás**. Un tresillo dentro de otro marco simple expresa una relación de *tuplet*, como `3:2`. Compartir una división ternaria no convierte ambos fenómenos en lo mismo.

### Si acentúas `2+2+2`, ¿has cambiado automáticamente a otro compás?

No. Puedes superponer una **reagrupación interna / agrupación** mediante acentos y conservar el 6/8 subyacente si siguen presentes sus dos pulsos compuestos.
...
No repitas tareas por obligación. Si la muestra principal y la explicación ya son coherentes, puedes omitir esta comprobación.

Si necesitas separar **lectura** de **representación**, escribe una célula nueva de un solo compás usando seis casillas `X/·`.

Condiciones:

- exactamente seis posiciones de corchea;
- organízalas conscientemente como `3 | 3`;
- puedes cambiar el número de ataques: lo importante es no añadir ni eliminar duración;
- antes de tocarla, marca los dos pulsos principales;
- explica por qué escribir seis casillas sigue sin ser, por sí solo, una definición universal de 6/8.

Esta microtarea no introduce una nueva transformación curricular. Sólo comprueba si la representación escrita conserva la jerarquía ya estudiada.

## 4. Validez de la evidencia y salud — 1 min

Una toma no es una evidencia limpia si dolor, hormigueo, entumecimiento, pérdida de fuerza o tensión persistente obligan a compensar el movimiento. En ese caso, registra la condición y aplica `DETENER CARGA` cuando corresponda; una señal de salud **no es un suspenso curricular**.

También invalida una conclusión amplia elegir una condición en la que tempo, lectura y ejecución colapsan simultáneamente. Reduce **una variable cada vez** —tempo, longitud o ayuda— y vuelve a obtener una muestra interpretable.
...
- sentir y mantener **dos pulsos principales** por compás de 6/8;
- conservar **tres subdivisiones internas** aproximadamente iguales dentro de cada pulso, también cuando alguna no suena;
- leer la muestra exclusiva sin playback previo organizándola como `2×3`, aunque existan errores locales;
- después del intento propio, escuchar la misma línea y marcar dos apoyos grandes por compás;
- explicar por qué **seis corcheas no bastan para definir la métrica**;
- distinguir el contraste prototípico `6/8 = 2×3` de `3/4 = 3×2`;
- distinguir las subdivisiones ordinarias de un compás compuesto de un **tresillo como tuplet en otro marco**;
- distinguir **reagrupación / agrupación** de cambio de compás;
- identificar la variable limitante dominante y una acción correctiva concreta si procede;
- no presentar una señal de salud/carga que invalide la muestra.
