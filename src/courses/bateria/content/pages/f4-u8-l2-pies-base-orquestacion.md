---
contentId: bat-f4-u8-l2
courseId: bateria
phase: 4
unit: 8
unitSlug: fase-4-unidad-8
slug: pies-base-orquestacion
kind: lesson
order: 2
title: "Pies simples bajo manos orquestadas"
summary: "Mantén bombo 1/3 y hi-hat de pie 2/4 mientras las manos recorren una frase Unidad 2 ya conocida."
duration: 20–30 min
competencies: [B8, G5, A8, H4, H2, H3, C1, C2, D7, K2, K7]
rudiments: []
published: true
---

## Objetivo

Mantener una base de pies ya conocida debajo de una orquestación manual también conocida.

La novedad dominante sigue siendo **orquestar sin perder identidad**. Los pies no reciben vocabulario nuevo.

## EJERCICIO ORIGINAL CREADO PARA ESTE CURSO — ORQUESTACIÓN + BASE DE PIES

<div data-notation-score data-score-src="/bateria/notation/f4/u8/f4-u8-l2-orchestration-feet.musicxml" data-score-title="Lección 2 — Orquestación + base de pies" data-score-badge="EJERCICIO ORIGINAL CREADO PARA ESTE CURSO"></div>

Manos:

- ocho corcheas exactamente como en **Unidad 2, Lección 4**;
- R/L preservado;
- caja + tom agudo + tom grave;
- acentos preservados.

Pies:

- bombo: 1 y 3;
- pedal hi-hat: 2 y 4.

## Construcción

1. Recupera la frase de manos.
2. Toca sólo la base de pies.
3. Combina manos + bombo.
4. Añade el chick 2/4.
5. Recompón el patrón completo.

Si los pies hacen que desaparezca una nota manual, no inventes otra digitación de inmediato. Localiza primero la coincidencia problemática.

## AMPLIACIÓN — lectura sencilla de varias voces

**EJERCICIO ORIGINAL CREADO PARA ESTE CURSO — LECTURA MULTIVOZ DEL MATERIAL CONOCIDO.**

No es una prueba de primera vista: el material rítmico ya es conocido. La novedad es usar la partitura de drumset para seguir varias capas sin depender sólo de la descripción verbal.

1. Antes de reproducir el audio, localiza en la partitura la voz de manos y las dos capas de pies.
2. Señala dónde coinciden manos y bombo en 1 y 3, y dónde aparece el chick en 2 y 4.
3. Toca una pasada mirando la notación y conservando el pulso.
4. Si leer hace colapsar el patrón, vuelve a la construcción por capas y prueba de nuevo después.

Esta ampliación introduce lectura elemental de drumset; **no convierte D7 en requisito del checkpoint de U8**.

## Diagnóstico

`MANOS / PIE DERECHO / PIE IZQUIERDO / TRANSICIÓN DE SUPERFICIE / PULSO / BALANCE / TENSIÓN`

Aislar una coincidencia es un **correctivo**, no la nueva forma del ejercicio.

## Tempo

El archivo usa 120 BPM como referencia técnica. Ajusta el tempo real hasta poder escuchar y observar todas las capas.

**No existe BPM de aprobado.**

## MÍNIMO PARA AVANZAR

- la frase manual sigue reconocible con pies;
- bombo 1/3 y chick 2/4 permanecen suficientemente estables;
- el recorrido caja/toms no cambia accidentalmente de densidad;
- identificas una coincidencia problemática si aparece;
- puedes aislarla y recomponer;
- mantienes ergonomía y escucha segura.

## Registrar esta práctica

Anota `frase + pies → coincidencia frágil → correctivo → conjunto`.