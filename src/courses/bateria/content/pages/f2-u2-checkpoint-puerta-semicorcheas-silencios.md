---
contentId: bat-f2-u2-check
courseId: bateria
phase: 2
unit: 2
unitSlug: fase-2-unidad-2
slug: puerta-de-semicorcheas-y-silencios
kind: checkpoint
order: 5
...
- elige un tempo cómodo; el `120 BPM` del MusicXML es sólo referencia técnica de playback;
- puedes usar click convencional en negras;
- cuenta o canta `1 e & a` si lo necesitas;
- no uses playback ni practiques la línea por fragmentos.

Después:

1. pulsa **Empezar**;
2. localiza las cuatro negras y la rejilla de semicorcheas;
3. observa dónde faltan ataques sin borrar mentalmente esas posiciones;
...
- **PRECISIÓN:** qué ataques/silencios coincidieron con la notación;
- **CONTINUIDAD / RECUPERACIÓN:** si mantuviste la línea o pudiste reincorporarte;
- **REJILLA INTERNA:** si las cuatro posiciones siguieron presentes durante silencios;
- **AYUDAS:** click, conteo/canto u otra ayuda realmente usada;
- **VARIABLE LIMITANTE:** lectura, tiempo, oído, memoria, técnica, comprensión o carga.

Después del primer intento puedes usar playback como feedback. Esa repetición **ya no aporta evidencia de primera vista**.

## 2. Muestra B — OÍR → ESCRIBIR — 2–3 min

Escucha una célula de **dos pulsos**. No mires una solución antes de escribir.

Protocolo:

1. primera escucha: localiza pulso y rejilla;
2. segunda escucha si la necesitas: identifica ataques y silencios;
3. escribe primero una rejilla explícita `1 e & a | 2 e & a`;
4. marca `X` para ataque y `·` para silencio;
5. sólo después pulsa **Mostrar respuesta**.

<div data-rhythm-dictation data-bpm="58" data-subdivision="4" data-pattern="10100110" data-answer="X · X · | · X X · — dos pulsos completos: ataques en 1, &, e de 2 y & de 2."></div>

La caligrafía no es el criterio. Compara:

- si conservaste las **ocho posiciones**;
...
> **Si una semicorchea está en silencio, ¿dónde continúa la subdivisión?**

Una respuesta suficiente debe expresar que el silencio elimina el ataque, **no la posición temporal**. La rejilla `1 e & a` sigue organizando el pulso y permite localizar el siguiente ataque aunque una o varias posiciones no suenen.

Si puedes señalar una posición concreta —por ejemplo `e` o `a`— y mantenerla dentro del pulso sin tocarla, la explicación es más informativa que repetir una definición de memoria.
...
- la negra conserva una referencia razonablemente estable al cambiar densidad;
...
Registra las **CONDICIONES** reales de la muestra:
...
Antes de aumentar novedad comprueba si aparece alguna señal que obligue a reducir o detener carga:
...
Si una señal persiste, corresponde valoración sanitaria; el curso no la diagnostica.
...
- localizar pulso y rejilla de semicorcheas en material nuevo accesible;
- mantener internamente posiciones durante silencios;
- decodificar una célula sin memorizar primero su dibujo;
- alternar densidades binarias sencillas de forma lenta y consciente;
- representar por escrito una célula auditiva muy corta con correspondencia razonable;
- distinguir si el fallo principal fue lectura, tiempo, oído, memoria, técnica o comprensión;
- continuar con un correctivo localizado cuando la debilidad no invalida el contenido siguiente.
...
- **pulso estable, posiciones confundidas:** reduce longitud y externaliza la rejilla;
