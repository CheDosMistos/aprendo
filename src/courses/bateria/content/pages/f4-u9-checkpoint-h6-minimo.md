---
contentId: bat-f4-u9-check
courseId: bateria
phase: 4
unit: 9
unitSlug: fase-4-unidad-9
slug: h6-minimo
kind: checkpoint
order: 5
title: "Evaluación — fills MÍNIMO: fill y retorno"
summary: "Demuestra un fill sencillo dentro de una frase conocida y vuelve al groove en el tiempo 1 correcto sin usar BPM como puerta."
duration: 20–30 min
competencies: [H6, H5, F3, B8, G5, C1, C2, C3, K2, K5, K7]
rudiments: []
published: true
---

## Qué demuestra este Evaluación

El criterio global que puede cerrarse aquí es **fills MÍNIMO**:

> **Inserta fills sencillos sin perder el pulso.**

En esta unidad, “insertar” significa además conservar una forma básica y volver al siguiente tiempo 1.

## Elige un fill

No necesitas ejecutar los dos para aprobar el mínimo.

### Opción A — Fill de 1 tiempo

<div data-notation-score data-score-src="/bateria/notation/f4/u9/f4-u9-l1-one-beat-fill-return.musicxml" data-score-title="Unidad 9.evaluación — Fill A y retorno" data-score-badge="EJERCICIO ORIGINAL CREADO PARA ESTE CURSO"></div>

### Opción B — Fill de 2 tiempos

<div data-notation-score data-score-src="/bateria/notation/f4/u9/f4-u9-l2-two-beat-fill-return.musicxml" data-score-title="Unidad 9.evaluación — Fill B y retorno" data-score-badge="EJERCICIO ORIGINAL CREADO PARA ESTE CURSO"></div>

Antes de tocar, declara qué opción eliges y cuánto dura.

Después demuestra:

`GROOVE → FILL → 1 → GROOVE`.

## MÍNIMO PARA fills

Debe observarse razonablemente que:

- sabes dónde empieza el fill;
- declaras su duración antes de tocar;
- mantienes una referencia temporal reconocible;
- la subdivisión no colapsa sistemáticamente;
- el recorrido por superficies sigue siendo controlable;
- aterrizas en el tiempo 1 correcto;
- continúas el groove después;
- ante un error aislado intentas mantener o recuperar la referencia;
- distingues `ENTRADA / DURACIÓN / SUBDIVISIÓN / SUPERFICIE / 1 / RECUPERACIÓN`;
- aplicas un correctivo limitado y recompones;
- mantienes sonido, balance y relajación suficientes;
- mantienes ergonomía, carga y escucha segura.

**No existe BPM de aprobado.** Cero errores tampoco es requisito.

## Orquestación

Puedes usar la versión escrita o una variante de superficies trabajada en Lección 3, siempre que:

- el ritmo y la duración sigan claros;
- no añadas densidad sólo para impresionar;
- el cambio tímbrico no borre el retorno al 1.

La calidad de fills no se mide por cuántos toms o platos utilizas.

## independencia avanzada queda abierto

La ampliación de Lección 4 puede mostrar una primera exposición a un ostinato simple mientras las manos realizan el fill.

**No forma parte de este Evaluación y esta unidad NO certifica independencia avanzada.**

## 5/4 y 7/8

Pueden probarse como AMPLIACIÓN sólo si esas métricas ya están funcionales. No son requisito de fills MÍNIMO ni del Hito 5.

## NO CERTIFICA

- fills COMPETENTE/FUNCIONAL;
- independencia avanzada independencia;
- fills improvisados libres;
- repertorio;
- vocabulario estilístico;
- velocidad alta;
- 5/4 o 7/8;
- orquestación compleja.

## Decisión

- **AVANZA:** el fill elegido entra, dura y devuelve al groove con pulso/forma suficientemente reconocibles.
- **AVANZA CON CORRECTIVO:** el retorno es funcional, pero una transición o superficie necesita mantenimiento.
- **REPITE EL BLOQUE ESPECÍFICO:** el fill desplaza sistemáticamente el 1 o hace desaparecer la referencia; no es necesario repetir toda esta unidad.

La siguiente unidad integra fills + adaptación técnica entre superficies y kit/orquestación de rudimentos dentro del **Hito 5**.