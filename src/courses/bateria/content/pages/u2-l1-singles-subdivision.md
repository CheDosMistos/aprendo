---
contentId: bat-f1-u2-l1
courseId: bateria
phase: 1
unit: 2
unitSlug: unidad-2
slug: leccion-1-singles-subdivision
kind: lesson
order: 1
title: Lección 1 — Singles agrupados y subdivisión
summary: Introduce Single Stroke Four y Single Stroke Seven, recupera un rudimento de U1 y conserva el pulso al pasar de dos a tres partes iguales.
duration: 25–30 min
competencies: [A2, A3, A6, B1, C1, C2, C4, E2, F1, K2, K4, K6]
rudiments:
  - Single Stroke Four
  - Single Stroke Seven
published: true
---

## Objetivo

Construir una primera representación correcta de **Single Stroke Four** y **Single Stroke Seven** y distinguir una **agrupación de golpes** de la subdivisión que la contiene.

## 1. Recuperación de U1 — 2–3 min

Elige un rudimento de U1 que no hayas tocado inmediatamente antes. Intenta nombrarlo y ejecutarlo lentamente una vez. **Después** contrástalo con PAS.

No repases los seis: buscamos comprobar disponibilidad retenida, no hacer un examen.

## 2. Pulso constante, densidad distinta — 5–6 min

Mantén cuatro pulsos regulares. Primero coloca **dos golpes iguales por pulso** y después **tres golpes iguales por pulso**, sin cambiar la velocidad del pulso.

<div data-notation-score data-score-src="/bateria/notation/u2/binario-ternario.musicxml" data-score-title="Dos y tres partes por pulso" data-score-badge="EJERCICIO ORIGINAL CREADO PARA ESTE CURSO"></div>

La partitura incluye una primera ventana visual al tresillo. No necesitas leer tresillos con fluidez todavía: la lectura estructurada se profundizará en U3.

**Pregunta de control:** si pasas de 2 a 3 notas por pulso y el click sigue en el mismo sitio, ¿ha cambiado el pulso? La respuesta es **no**; ha cambiado la subdivisión.

## 3. Single Stroke Four — 6 min

Abre la fuente normativa y localiza **2. Single Stroke Four**:

[PAS — International Drum Rudiments](https://pas.org/wp-content/uploads/2024/04/pas-rudiments.pdf)

Antes de repetirlo mucho, observa:

- cómo se agrupan los golpes;
- dónde termina el grupo respecto a la referencia temporal;
- las dos posibilidades de mano inicial indicadas por PAS.

Trabaja despacio. Si el final del grupo llega tarde, prueba primero si el problema es de subdivisión o de tensión antes de intentar acelerar.

## 4. Single Stroke Seven — 6 min

Localiza **3. Single Stroke Seven** en la misma fuente PAS.

Compara con Single Stroke Four sin asumir que “más golpes” significa simplemente “lo mismo más rápido”. Observa la estructura escrita y su resolución.

Prueba ambos inicios cuando resulte cómodo y escucha si una mano cambia claramente el sonido o el tiempo.

## 5. Oído y teoría — 3–4 min

Con una referencia auditiva independiente —por ejemplo el metrónomo usando subdivisión o una demostración— escucha una secuencia y decide si percibes **2 o 3 partes por pulso** antes de mirar la respuesta.

Puedes explicar estos tres términos así:

- **pulso:** referencia regular;
- **subdivisión:** división interna del pulso;
- **agrupación:** forma de organizar golpes dentro de esa referencia.

No son sinónimos.

## Diagnóstico K2

Si un grupo termina tarde:

**síntoma → hipótesis → prueba**

Por ejemplo: “el último golpe cae detrás del click → quizá estoy cerrando el grupo de forma irregular → vuelvo a dos/tres partes iguales por pulso y compruebo si el problema sigue ahí”.

## Rutina estándar

- 3 min recuperación U1;
- 6 min contraste 2/3 + técnica;
- 6 min Single Stroke Four;
- 6 min Single Stroke Seven;
- 3–4 min oído/teoría;
- 2 min registro digital.

El BPM es una **condición de práctica**, no un criterio de aprobado.