---
contentId: bat-f5-u5-check
courseId: bateria
phase: 5
unit: 5
unitSlug: fase-5-unidad-5
slug: h7-minimo
kind: checkpoint
order: 5
title: "Evaluación — independencia avanzada MÍNIMO"
summary: "Comprueba que puedes mantener un ostinato sencillo mientras otra voz varía en una tarea musical concreta, sin convertirlo en una prueba de independencia avanzada."
duration: 20–30 min
competencies: [H7, H5, C2, I2, A5, K2, K5, K7]
rudiments: []
published: true
---

## Qué certifica

Esta evaluación puede certificar:

> **independencia avanzada MÍNIMO: mantiene un ostinato simple mientras otra voz varía en una tarea musical concreta.**

No certifica independencia avanzada.

## Muestra

<div data-notation-score data-score-src="/bateria/notation/f5/u5/f5-u5-checkpoint-h7.musicxml" data-score-title="Unidad 5 — Evaluación independencia avanzada MÍNIMO" data-score-badge="EJERCICIO ORIGINAL CREADO PARA ESTE CURSO"></div>

La combinación es nueva respecto a Lección 3, pero utiliza vocabulario conocido. No se presenta como examen de primera vista.

## Condición

- aprende primero la capa fija: hi-hat + backbeat;
- estudia después el bombo de A y B;
- cuando cada parte sea comprensible, realiza bloques `A → B → A → B`;
- el tempo debe permitir observar independencia real, no supervivencia por tensión.

## Evidencia mínima

Debe observarse razonablemente que:

1. la capa fija conserva pulso y función mientras cambia el bombo;
2. el cambio A/B no obliga a detenerse sistemáticamente;
3. una entrada de bombo imperfecta no destruye automáticamente el ostinato;
4. puedes recuperar primero la referencia estable y después el detalle;
5. el balance entre voces sigue controlado de forma básica;
6. identificas verbalmente qué voz permanece y cuál varía;
7. la ejecución no depende de tensión creciente ni de una cifra de BPM;
8. mantienes escucha segura.

## Si falla

Usa la cadena del sistema de práctica:

`síntoma → hipótesis → prueba → correctivo → recomposición → nueva pasada`.

No repitas toda esta unidad si el fallo está localizado en una sola coincidencia o transición.

## CERTIFICA

- **independencia avanzada MÍNIMO en contexto musical preparado.**

## NO CERTIFICA

- independencia avanzada COMPETENTE/FUNCIONAL global;
- independencia a cuatro extremidades avanzada;
- sistemas de Chapin, New Breed o 4-Way Coordination;
- ghost notes como requisito;
- funk/soul/R&B dominados;
- linear drumming;
- velocidad prefijada.

**No existe BPM de aprobado.**

## Decisión

- **AVANZA:** la capa estable sobrevive a la variación y existe recuperación funcional.
- **AVANZA CON CORRECTIVO:** la función se conserva, pero una transición o balance requiere mantenimiento.
- **REPITE BLOQUE ESPECÍFICO:** la variación derriba sistemáticamente el ostinato; aisla esa relación y vuelve a recomponer.

Unidad 6 cambia el foco a sonido funcional, afinación básica, click y grabación.
