---
contentId: bat-f3-u5-overview
courseId: bateria
phase: 3
unit: 5
unitSlug: fase-3-unidad-5
slug: fase-3-unidad-5-introduccion
kind: unit
order: 0
title: "Motivo, identidad, repetición y contraste"
summary: "Pasa de reconocer motivos a crearlos: construye una idea breve rastreable, repítela, contrástala, hazla volver y explica qué mantiene su identidad."
duration: Unidad flexible · 4 lecciones + Puerta G1
competencies: [G1, C1, D6, F3, D1, D2, D3, D4, D5, E3, E4, E5, K2, K5, K6, K8, B7]
rudiments: []
published: true
---

U4 enseñó a **reconocer** motivos y relaciones dentro de una forma. U5 cambia el papel: ahora tienes que **crear una idea que pueda sobrevivir a la memoria, la repetición y el contraste**.

`CREAR → TOCAR → ESCRIBIR → REPETIR → ESCUCHAR → COMPARAR → CONTRASTAR → RETORNAR → EXPLICAR`

> **Novedad dominante:** identidad motívica bajo repetición y contraste.

## ESENCIAL AHORA

- crear una célula breve reconocible;
- repetirla sin modificarla accidentalmente;
- escribirla de forma que puedas recuperarla;
- distinguir `MISMO / RELACIONADO / NUEVO`;
- crear contraste sin confundirlo con “más notas”;
- volver a A después de B;
- usar pregunta/respuesta como relación pedagógica;
- explicar qué rasgos sostienen la identidad.

## No existe un porcentaje mágico de identidad

No hay una regla universal del tipo “si conserva X % de las notas sigue siendo el mismo motivo”. La relación puede apoyarse en ataques y silencios, duraciones, densidad, agrupación, acentos u otros rasgos relevantes.

La pregunta útil es:

> **¿qué se conservó y qué cambió?**

Si no puedes rastrear ninguna relación perceptible, el material puede ser perfectamente válido como idea nueva, pero no se llama automáticamente desarrollo del motivo anterior.

## Frontera con U6

U5 trabaja **G1 — motivo, repetición y contraste**. Puede mostrar A/A’ y una variación local pequeña para discutir identidad, pero **no sistematiza todavía** desplazamiento, reagrupación, aumentación/disminución, retrogradación, cambios de sticking u otras operaciones de G2.

Eso comienza en U6.

## Arquitectura

### L1 — Crear un motivo que puedas reconocer
Estudias tres modelos breves y construyes un motivo propio que puedas tocar, escribir y recuperar.

### L2 — Identidad: mismo, relacionado o nuevo
Comparas A con dos candidatos y justificas la relación sin recurrir a porcentajes arbitrarios.

### L3 — Repetición, contraste y retorno
Usas `A → A → B → A` como andamiaje para construir una frase con identidad y arco simple.

### L4 — Pregunta, respuesta y memoria motívica
Mantienes una pregunta A recuperable después de una respuesta y separas memoria, continuidad y relación musical.

### Checkpoint — Puerta G1 hacia transformación
Creas material propio nuevo, demuestras repetición/contraste/retorno y clasificas una variante externa antes de entrar en G2.

## Evaluación

La creatividad no se puntúa por rareza ni por cantidad de notas. Se observan por separado:

- reconocibilidad;
- intención;
- repetición/contraste;
- continuidad temporal;
- legibilidad;
- recuperación;
- relación pregunta/respuesta;
- explicación de identidad;
- diferencia entre decisión consciente y accidente.

**No hay un corrector automático de creatividad.** Aprendo aporta ejemplos, reproducción, notación, grabación y criterios observables; la decisión musical debe quedar explicada, no reducida a una respuesta secreta de la interfaz.

## Regla de avance

Para entrar en U6 no necesitas una obra compleja. Necesitas **un motivo rastreable** sobre el que una transformación consciente pueda operar.
