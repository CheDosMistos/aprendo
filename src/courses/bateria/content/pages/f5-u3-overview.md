---
contentId: bat-f5-u3-overview
courseId: bateria
phase: 5
unit: 3
unitSlug: fase-5-unidad-3
slug: fase-5-unidad-3-introduccion
kind: unit
order: 0
title: "Backbeat, corchea, energía y forma"
summary: "Usa un backbeat binario como infraestructura para trabajar feel, energía, dinámica y forma sin reducir rock, pop, punk o metal a un único patrón."
duration: Unidad flexible · 4 lecciones
competencies: [I2, H5, H6, A5, C2, F3, I3, I4, K2, K5, K7]
rudiments: []
published: true
---

## Novedad dominante: blues y shuffle / groove y variaciones en familia de backbeat

esta unidad abre la primera familia estilística focal de Fase 5.

> **ESTILO ≠ PATRÓN.**

El groove de corcheas y backbeat que usamos como CORE es un **arquetipo pedagógico** útil para entrar en una gran cantidad de música rock/pop, no una definición universal del género.

La pregunta de la unidad es musical:

> **¿Puedes sostener un groove binario dentro de una forma conocida, cambiar energía y dinámica con intención y continuar cuando aparecen cues o transiciones?**

## Groove CORE

**EJERCICIO ORIGINAL CREADO PARA ESTE CURSO**

- 4/4;
- hi-hat cerrado en corcheas;
- caja en 2 y 4;
- bombo en 1 y 3;
- dinámica inicial `mf`;
- articulación controlada.

No existe rimshot obligatorio ni BPM objetivo de aprobado.

## Energía no es velocidad

En esta unidad tratamos la energía como una relación entre:

`CONTINUIDAD / ARTICULACIÓN / DINÁMICA / DENSIDAD / SUPERFICIE / FORMA`.

Por tanto:

`ENERGÍA ≠ BPM ALTO ≠ VOLUMEN MÁXIMO`.

## Familias

### CORE — rock/pop como familia de backbeat
Usamos subdivisión binaria, backbeat, dinámica de secciones y relación con forma.

### Ventana punk
Trabajamos continuidad, economía de movimiento y función del backbeat. Punk **no se define aquí como “tocar lo más rápido posible”**.

### AMPLIACIÓN — ventana metal
Sólo articulación/energía sobre material conocido. **No exige doble pedal, blast beat ni patrones rápidos de bombo.**

## Arquitectura

### Lección 1 — Backbeat y rejilla de corcheas
Construir el groove CORE por capas y sostenerlo sin fills.

### Lección 2 — Energía, articulación y dinámica
Cambiar una variable expresiva sin acelerar ni tensarse.

### Lección 3 — Ventana punk: continuidad y economía
Sostener bloques largos con eficiencia y un Fill A conocido cuando tenga función.

### Lección 4 — Pieza A: forma y decisiones de energía
Aplicar esta unidad a Pieza A con Chart A, dinámica y cues ya conocidos.

## Salida de esta unidad aporta evidencia hacia **blues y shuffle MÍNIMO** y profundiza groove y variaciones dentro de la familia realmente estudiada, pero **no declara rock, pop o punk dominados, ni metal funcional, ni groove y variaciones COMPETENTE/FUNCIONAL global**.

No se crea un Evaluación global nuevo: la arquitectura superior no lo prescribe.

**No existe BPM de aprobado.** gestión de carga y salud y la escucha segura permanecen activos.