---
contentId: bat-f2-u5-l3
courseId: bateria
phase: 2
unit: 5
unitSlug: fase-2-unidad-5
slug: 2-3-4-reorganizar-densidad
kind: lesson
order: 3
title: "2 ↔ 3 ↔ 4: reorganizar densidad"
summary: "Añade cuatro subdivisiones por pulso y encadena 2↔3↔4 lentamente, manteniendo una referencia temporal continua y separando densidad, tempo, relajación y exactitud interna."
duration: 25–30 min
competencies: [A1, A2, C1, C2, C3, D1, D2, D3, F1, F2, K2, K4, K6]
rudiments: []
published: true
---

Lección 2 demostró que el pulso puede continuar mientras su interior cambia `2 ↔ 3`. Lección 3 añade una tercera organización conocida de fases anteriores —**cuatro partes iguales**— y la integra en la misma lógica.

> **MÁS ATAQUES DENTRO DEL PULSO NO SIGNIFICA MÁS TEMPO. EL PULSO SIGUE SIENDO LA REFERENCIA.**

La novedad dominante es **encadenar `2 ↔ 3 ↔ 4` de forma lenta y consciente**. No buscamos velocidad máxima, sextillos ni una métrica nueva. Seguimos en 4/4; el compás compuesto será Unidad 6.

**ESENCIAL AHORA:** sostener pulso interno mientras cambia la densidad interna del pulso.

**AMPLIACIÓN:** reducir la longitud de bloques previsibles sin subir tempo.

**AVANZADO:** usar secuencias menos predecibles manteniendo igualdad, sonido y relajación.

## 1. Recuperar 2 ↔ 3 — 3 min

Haz una única pasada breve de Lección 2:

`2 2 2 2 | 3 3 3 3 | 2 2 2 2`

Cada número indica **cuántas partes iguales contiene cada negra**, no cuántos pulsos hay.

Comprueba sólo:

- que la negra conserva una distancia reconocible;
- que el cambio de 2 a 3 no arrastra el BPM;
- que puedes volver a 2 sin frenar;
- que distingues **pulso** de **densidad interna**.

Si hoy esta recuperación ya desorganiza pulso interno, alarga los bloques y reduce tempo antes de añadir `4`. No necesitas “aprobar de nuevo” Lección 2: necesitas una condición válida para introducir una sola capa nueva.

## 2. Añadir cuatro partes sin mover la negra — 5–6 min

Mantén la misma negra y distribuye ahora cada pulso en **cuatro partes iguales**.

Puedes recuperar temporalmente el conteo `1 e & a`, pero úsalo como representación, no como una orden de acelerar.

Compara un solo pulso de cada organización:

- `2`: dos partes iguales;
- `3`: tres partes iguales;
- `4`: cuatro partes iguales.

En todos los casos la duración exterior es **una negra completa**.

Haz cuatro pulsos sólo de `4` y observa:

- igualdad entre las cuatro posiciones;
- estabilidad del pulso;
- altura y sonido entre manos;
- si aparece tensión por intentar “meter” cuatro golpes a la fuerza.

### Bilateralidad

Primero usa alternancia continua iniciando con derecha; después repite iniciando con izquierda. No conviertas el sticking en la tarea principal. La pregunta es si la tercera densidad puede aparecer **sin deformar el tiempo ni cargar una mano**.

Si cuatro partes son desiguales, no practiques todavía el cambio triple. Estabiliza unos pocos pulsos de `4`, reduce tempo y vuelve al encadenamiento cuando la información sea diagnosticable.

## 3. Leer 2 ↔ 3 ↔ 4 — 10–11 min

`EJERCICIO ORIGINAL CREADO PARA ESTE CURSO`

<div data-notation-score data-score-src="/bateria/notation/f2/u5/f2-u5-l3-2-3-4.musicxml" data-score-title="Unidad 5.Lección 3 — 2 ↔ 3 ↔ 4: reorganizar densidad" data-score-badge="EJERCICIO ORIGINAL CREADO PARA ESTE CURSO" data-score-feedback="after-attempt" data-score-source-url="/bateria/notation/f2/u5/f2-u5-l3-2-3-4.musicxml" data-score-source-label="MusicXML — fuente del ejercicio"></div>

Antes de **Habilitar audio**, localiza las tres densidades y señala dónde se producen los cambios. Haz una pasada propia a tempo cómodo.

### Una misma duración, tres repartos

El MusicXML usa `divisions=12`, de modo que una negra dura `12` unidades:

- **2:** `6 + 6 = 12` → dos corcheas binarias;
- **3:** `4 + 4 + 4 = 12` → tresillo real `3:2`;
- **4:** `3 + 3 + 3 + 3 = 12` → cuatro semicorcheas.

> **2, 3 y 4 cambian el reparto interior; las tres opciones ocupan exactamente el mismo pulso.**

La partitura progresa así:

1. compás completo en `2`;
2. compás completo en `3`;
3. compás completo en `4`;
4. cuatro pulsos consecutivos con `2 → 3 → 4 → 3`.

El último compás es el núcleo de transferencia de esta lección: la negra no debe encogerse ni expandirse al cambiar la cantidad de ataques.

### Protocolo

1. **LEE:** identifica `2`, `3` o `4` antes del bloque;
2. **MARCA:** conserva internamente la negra;
3. **EJECUTA:** alternancia sencilla, con recorrido cómodo;
4. **CONTINÚA:** un error interior no obliga a abandonar el pulso siguiente;
5. **COMPARA:** tras la primera pasada, habilita audio y escucha pulso vs. igualdad interna por separado.

El `<sound tempo="120"/>` es una referencia técnica de reproducción. **No es un BPM de aprobación ni una recomendación universal de práctica.**

## 4. De bloques previsibles a cambios menos predecibles — 5–6 min

Empieza con una secuencia que ya conoces:

`2 | 3 | 4 | 3`

Cada símbolo dura **un pulso**. Repite hasta que puedas describir qué variable falla si se rompe.

Después elige **una** de estas rutas y no mires más allá del próximo pulso:

- A: `2 | 4 | 3 | 2`;
- B: `3 | 2 | 4 | 3`;
- C: `4 | 3 | 2 | 4`.

No hace falta convertirlo en azar perfecto. El objetivo es reducir la anticipación motora sin añadir todavía una tarea de primera vista formal.

Para cada secuencia registra tres capas:

- **PULSO:** ¿la negra permanece estable?;
- **subdivisión binaria y ternaria/INTERIOR:** ¿cada bloque conserva partes razonablemente iguales?;
- **TÉCNICA:** ¿sonido, bilateralidad y relajación sobreviven al cambio?

Si colapsa pulso interno, **reduce longitud o densidad antes de subir tempo**. Si pulso interno sobrevive pero una densidad concreta se deforma, trabaja esa organización de forma aislada durante unos pulsos y vuelve a insertarla.

### No conviertas densidad en velocidad

A cuatro subdivisiones por pulso hay más ataques por segundo que a dos **si el pulso mantiene el mismo BPM**, pero el tempo de referencia no ha cambiado. Esa diferencia de densidad es precisamente lo que debes aprender a reorganizar.

Subir el BPM simultáneamente impediría saber si el fallo proviene de la nueva densidad o del cambio de tempo. En esta lección, modifica una variable principal cada vez.

## 5. Registro y decisión — 2–3 min

Usa:

`INFERENCIA → EVIDENCIA → TAREA → CONDICIONES → DECISIÓN`.

Registra:

- BPM y unidad de pulso como condición;
- secuencia usada;
- estabilidad del pulso;
- igualdad de `2`, `3` y `4` por separado;
- exactitud de los cambios;
- continuidad tras error local;
- mano de inicio;
- sonido y relajación;
- ayudas usadas;
- variable limitante principal;
- decisión: `CONTINUAR`, `CONTINUAR + CORRECTIVO`, `REDUCIR NOVEDAD` o `DETENER CARGA`.

Completar la página **no actualiza automáticamente pulso interno, subdivisión binaria y ternaria, cambios de subdivisión, tuplets y ornamentación ni ninguna otra competencia**. El BPM describe la condición; **BPM es condición, no competencia**.

## MÍNIMO PARA AVANZAR A Lección 4

Puedes abrir Lección 4 cuando, con una condición cómoda:

- sostienes una referencia de pulso reconocible al encadenar `2 ↔ 3 ↔ 4` lentamente;
- realizas cada densidad con una igualdad interna suficiente para distinguir qué parte se deforma;
- completas al menos una secuencia previsible y otra menos predecible sin perder sistemáticamente pulso interno;
- puedes continuar o recuperarte después de un error local;
- repites iniciando con ambas manos sin que un lado destruya de forma sistemática sonido o tiempo;
- explicas por qué **más densidad no equivale a más tempo**.

No se exige:

- cero errores;
- un BPM concreto o alto;
- cambios rápidos;
- dominar todas las secuencias propuestas;
- sextillos;
- 6/8;
- primera vista formal forma y lectura de chart;
- retirar todas las ayudas.

## COMPETENTE / FUNCIONAL EN ESTA TAREA

La tarea empieza a ser funcional cuando puedes reorganizar `2 ↔ 3 ↔ 4` en bloques cortos sin que la negra cambie sistemáticamente de velocidad, identificar qué densidad o transición limita el resultado y mantener control técnico suficiente en ambas direcciones.

Eso **no declara cambios de subdivisión ni tuplets y ornamentación FUNCIONALES globalmente**. Lección 4 cambiará el tipo de representación —oír, escribir, leer y transformar— y el Evaluación decidirá si la organización temporal está suficientemente disponible para abrir Unidad 6.

## AVANZADO EN ESTA TAREA

Como ampliación, cambia de densidad cada pulso usando una de las rutas A–C y retira sólo una pista verbal. Mantén el mismo tempo.

No añadas sextillos: **Unidad 8** los tratará como vocabulario propio. No conviertas el tresillo en 6/8: **Unidad 6** cambiará explícitamente el marco métrico. No uses click reducido ni gaps: **Unidad 10** tiene esa función.

## Si algo se rompe

- **`4` acelera la negra:** vuelve a cuatro pulsos aislados de semicorcheas con click en negras;
- **`3` se aplana al venir de `4`:** canta tres posiciones antes de tocar el cambio;
- **el cambio correcto llega tarde:** alarga cada bloque a dos o cuatro pulsos antes de volver al cambio por pulso;
- **pulso interno colapsa con una secuencia menos predecible:** vuelve a la ruta conocida; reduce novedad, no subas BPM;
- **una mano endurece el patrón:** baja altura, tempo o longitud y compara inicios R/L;
- **el error local arrastra todo el compás:** recupera en el próximo pulso identificable en vez de reiniciar desde cero;
- **aparece dolor, hormigueo, entumecimiento, pérdida de fuerza o tensión persistente:** reduce o detén la carga según el sistema general.

La siguiente lección cambiará de representación: **OÍR → ESCRIBIR → LEER → TRANSFORMAR** material 2–3–4 sin afirmar equivalencia de ataques cuando cambia la densidad.
