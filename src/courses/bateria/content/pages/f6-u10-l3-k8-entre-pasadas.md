---
contentId: bat-f6-u10-l3
courseId: bateria
phase: 6
unit: 10
unitSlug: fase-6-unidad-10
slug: k8-entre-pasadas
kind: lesson
order: 3
title: "Entre pasadas: K8 aplicado al cuello de botella real"
summary: "Usa la autonomía funcional de U9 para escoger qué corregir, qué apoyo mantener y qué problema dejar temporalmente sin tocar."
duration: 25–30 min
competencies: [K8, K2, K5, I4, D7, I6, H5, H6]
rudiments: []
published: true
---

## Objetivo

Aplicar K8 dentro de un proyecto musical real sin convertir U10 en otra unidad de «aprender a practicar».

U9 ya desarrolló:

`PROBLEMA → HIPÓTESIS → ESTRATEGIAS PLAUSIBLES → RECURSO → ELECCIÓN → EVIDENCIA → MANTENER / CAMBIAR / RETIRAR`

Ahora esa cadena tiene una restricción nueva:

> **NO TODO LO QUE FALLÓ EN LA PASADA MERECE SER CORREGIDO ANTES DE LA SIGUIENTE.**

## Prioridad por función

### DECISIÓN CURRICULAR RAZONADA

Ordena problemas según cuánto comprometen el proyecto:

1. **estructura/pulso:** pierdes forma o referencia temporal;
2. **transición/cue:** la pieza continúa pero una decisión crítica llega tarde o no llega;
3. **función estilística:** el groove permanece, pero pierde carácter o relación musical;
4. **detalle:** ornamentación, fill o articulación local que no compromete lo anterior.

No es una escala científica universal. Es una jerarquía curricular para evitar dedicar veinte minutos a pulir un fill mientras la forma sigue colapsando.

## EJERCICIO ORIGINAL CREADO PARA ESTE CURSO — «Una intervención, una pasada»

1. Revisa la línea base de L2.
2. Elige **un cuello de botella dominante**.
3. Formula una hipótesis.
4. Elige una estrategia o apoyo ya conocido.
5. Trabaja el problema en modo laboratorio.
6. Vuelve pronto a `ANTES → PROBLEMA → DESPUÉS`.
7. Decide qué harás con el apoyo en la próxima pasada:
   - `MANTENER`;
   - `REDUCIR`;
   - `RETIRAR`;
   - `CAMBIAR`.
8. Deja por escrito al menos un problema secundario que **no** vas a trabajar todavía.

## Apoyos legítimos

Un apoyo puede ser:

- más información en el chart;
- una marca visual temporal;
- click más explícito;
- reducción de densidad;
- ensayo aislado de una transición;
- grabación;
- una pista/stem si responde a la pregunta;
- feedback de otra persona.

Un apoyo no es «hacer trampa». El problema es mantenerlo sin saber para qué sirve o retirarlo antes de que puedas interpretar lo que ocurre.

## Retirar apoyos

No retires algo sólo porque «deberías poder hacerlo solo».

Pregunta:

1. ¿qué información me da este apoyo?
2. ¿puedo recuperar esa información desde otra fuente?
3. ¿qué predigo que ocurrirá si lo reduzco?
4. ¿cómo sabré si la retirada ayudó o sólo aumentó la carga?

## Qué NO hace L3

- no enseña de nuevo interleaving;
- no enseña de nuevo feedback;
- no crea una teoría nueva de autorregulación;
- no exige retención/transferencia cuando no responden a la pregunta;
- no certifica K8 AVANZADO global.

## Rutinas

**MÍNIMA:** seleccionar un problema y justificar una intervención.

**ESTÁNDAR:** laboratorio corto + retest de transición + decisión sobre el apoyo.

**AMPLIADA:** comparar dos estrategias plausibles si la primera evidencia queda ambigua.

## Criterio de salida

Continúa cuando:

- has priorizado por función musical;
- puedes explicar por qué **no** trabajas otros detalles todavía;
- el correctivo vuelve al contexto;
- sabes qué apoyo usarás en la próxima pasada y con qué propósito.
