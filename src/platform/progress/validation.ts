...
export function validateRecordExecutionInput(value: unknown): RecordExecutionInput {
  if (!isRecord(value)) {
    throw new ProgressValidationError('Execution payload must be an object.');
  }

  const courseId = requiredIdentifier(value.courseId, 'courseId');
  const contentId = requiredIdentifier(value.contentId, 'contentId');

  if (!practiceResults.has(value.result as PracticeResult)) {
    throw new ProgressValidationError('Invalid practice result.');
  }

  if (!healthSignals.has(value.health as HealthSignal)) {
    throw new ProgressValidationError('Invalid health signal.');
  }
...
function requiredIdentifier(value: unknown, field: string): string {
  if (typeof value !== 'string') {
    throw new ProgressValidationError(`${field} must be a string.`);
  }

  const normalized = value.trim();
  if (!identifierPattern.test(normalized)) {
    throw new ProgressValidationError(`Invalid ${field}.`);
  }
...
function optionalText(value: unknown, maxLength: number, field: string): string | undefined {
  ...
  if (typeof value !== 'string') {
    throw new ProgressValidationError(`${field} must be text.`);
  }

  const normalized = value.trim();
  if (normalized.length === 0) return undefined;
  if (normalized.length > maxLength) {
    throw new ProgressValidationError(`${field} is too long.`);
  }
